        ...
        function confirmarReinicio() {
            // Limpia arrays en memoria
            palabras = [];
            jugadores = [];

            // Limpia localStorage (NUEVO)
        ...
        /* --- FUNCIÓN AUXILIAR: FISHER-YATES SHUFFLE (NUEVO) --- */
        ...
        /* --- GESTIÓN DE DATOS CON LOCALSTORAGE (NUEVO) --- */
        ...
        function normalize(str) {
            ...
            // Ponemos el texto y activamos la animación de "bajada"
            errorElem.innerText = message;
            errorElem.classList.add('show');

            // Esperamos 3 segundos antes de iniciar la "subida" (desvanecimiento)
            errorElem.timer = setTimeout(() => {
                errorElem.classList.remove('show');

                // Esperamos 500ms (lo que dura la animación CSS) para borrar el texto
                setTimeout(() => {
        ...
        function agregarPalabra() {
            ...
            errorElem.innerText = ''; // Limpiar error anterior
            ...
            // Validar solo letras (incluyendo acentos y espacios)
            ...
            // Validar duplicados (case-insensitive y sin acentos)
        ...
        function nuevaRonda() {
            ...
            // --- FIX DEFINITIVO PARA ANIMACIÓN (HARD RESET) ---
            // Reemplazamos el elemento completo para que el navegador olvide 
            // cualquier estado de animación anterior.
            const oldWrapper = document.querySelector('.timer-wrap');
            if (oldWrapper) {
                const newWrapper = oldWrapper.cloneNode(true);
                // Aseguramos que el clon nace limpio de clases críticas
        ...
        /* --- NUEVA FUNCIÓN OPTIMIZADA (LATE TODO JUNTO) --- */
        function actualizarTimerUI(segundos) {
            const numElem = document.getElementById('timerNumber');
            const circle = document.getElementById('timerCircle');
            const wrapper = document.querySelector('.timer-wrap'); // SELECCIONAMOS EL CONTENEDOR
            ...
            if(circle) {
                ...
                // --- LÓGICA DE LOS 15 SEGUNDOS ---
                if (segundos <= 15) {
                    // ACTIVAMOS PÁNICO EN EL CONTENEDOR (Todo late junto)
